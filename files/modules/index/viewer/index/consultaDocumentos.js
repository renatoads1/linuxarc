...
//  enviar para action zipa download como um array de nomes de img   
    var ids = $('.staGrid').staGridSelectedRows().map((linha)=>linha.id);
    var caminhos = $('.staGrid').staGridSelectedRows().map((linha)=>linha.caminho);
    trs = $("tbody tr");

    windowOpenWithPost(url_raiz_empresa + "index/index/arq_down_zip", {files: caminhos,idarq:ids});
        setTimeout(function () {
        $('.staGrid').staGrid({forceRefresh: true});
    },1000);
}
function contabilizaEmLote(){
    var ids = $('.staGrid').staGridSelectedRows().map((linha)=>linha.id);
   
 $.post(url_raiz_empresa + "index/index/contabiliza_em_lote",{idarq:ids}, function(data,status){
            alert("Data: " + data + "\nStatus: " + status);
        });
               
    //windowOpenWithPost(url_raiz_empresa + "index/index/contabiliza_em_lote", {idarq:ids});
